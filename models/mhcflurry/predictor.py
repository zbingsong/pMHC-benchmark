...
def check_model_loaded(func: typing.Callable) -> typing.Callable:
    def wrapper(*args, **kwargs):
        if MHCflurryPredictor._predictor is None:
            MHCflurryPredictor.load()
...
class MHCflurryPredictor(BasePredictor):
    ...
    def run(
    ...
    ) -> tuple[tuple[list[torch.DoubleTensor], list[torch.DoubleTensor]], list[torch.LongTensor], list[torch.DoubleTensor]]:
        affinity_preds = []
        presentation_preds = []
        labels = []
        log50ks = []
        ...
        return (presentation_preds, affinity_preds), labels, log50ks
