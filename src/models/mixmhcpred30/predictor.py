...
class MixMHCpred30Predictor(BasePredictor):
    ...
    def run_retrieval(
        ...
        for mhc_name, group in df:
            ...
            with open(f'peptides_mixmhcpred30.fasta', 'w') as f:
                for peptide in peptides:
                    f.write(f'>{peptide}\n{peptide}\n')
                    
            start_time = time.time_ns()
            run_result = subprocess.run([cls._executable, '-i', 'peptides_mixmhcpred30.fasta', '-o', 'result_mixmhcpred30.tsv', '-a', mhc_formatted], stdout=subprocess.DEVNULL)
            end_time = time.time_ns()
            assert run_result.returncode == 0
            times.append(end_time - start_time)
            try:
                result_df = pd.read_csv('result_mixmhcpred30.tsv', sep='\t', skiprows=list(range(11)))
    ...
    def run_sq(
        ...
        start_time = time.time_ns()
        run_result = subprocess.run([cls._executable, '-i', 'peptides_mixmhcpred30.fasta', '-o', 'result_mixmhcpred30.tsv', '-a', ','.join(mhcs_formatted)], stdout=subprocess.DEVNULL)
        end_time = time.time_ns()
        assert run_result.returncode == 0
        times.append(end_time - start_time)
        try:
    ...
    def run_sensitivity(
        ...
        for mhc_name, group in df:
            ...
            run_result = subprocess.run([cls._executable, '-i', 'peptides_mixmhcpred30.fasta', '-o', 'result_mixmhcpred30.tsv', '-a', mhc_formatted])
            assert run_result.returncode == 0
            result_df = pd.read_csv('result_mixmhcpred30.tsv', sep='\t', skiprows=list(range(11)))
