...
class TripHLApanPredictor(BasePredictor):
    '''
    Unusable for now.
    TripHLApan does not check every peptide-MHC pair, and the filtering mechanism is unknown
    '''
    tasks = None
    _exe_dir = None
    _temp_dir = None
    ...
    def load(cls, predictor_configs: PredictorConfigs) -> None:
        ...
        cls.tasks = ['EL']
        cls._wd = os.getcwd()
        cls._exe_dir = '~/repo/TripHLApan/TripHLApan/codes'
    ...
    def run_retrieval(
        ...
        inputs = [f'{peptide}\t{mhc}\n' for peptide, mhc in zip(peptides, mhcs)]
        input_path = os.path.join(cls._exe_dir, '../for_prediction/triphlapan_in.txt')
        with open(input_path, 'w') as f:
            f.writelines(inputs)
        
        start_time = time.time_ns()
        # run_result = subprocess.run(['../../env/bin/python', 'independent_test.py', '../for_prediction/triphlapan_in.txt'], cwd=cls._exe_dir, stdout=subprocess.DEVNULL)
        end_time = time.time_ns()
        # assert run_result.returncode == 0
        times.append(end_time - start_time)
        try:
            output_path = os.path.join(cls._exe_dir, '../for_prediction/outputs/triphlapan_in.txt')
            with open(output_path, 'r') as f:
                results = [float(line.strip()) for line in f]
            assert len(results) == len(df), f'Length mismatch: {len(results)} vs {len(df)}'
        except Exception as e:
            raise e
        
        result_df = pd.DataFrame({'score': results, 'mhc': df['mhc_name'], 'len': df['peptide'].str.len()})
        ...
        for mhc_name, group in result_df.groupby('mhc'):
            ...
            for length, subgroup in grouped_by_len:
                ...
                # TripHLApan produces a score in (0, 1) where higher score => better epitope
                pred[length] = torch.tensor(subgroup['score'].tolist(), dtype=torch.double)
